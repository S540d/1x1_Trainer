...
- **Code-Audit #357, Punkt 2 erledigt (PR #361, 2026-09-06):** `saveOperation`/`getOperation` (toter Code, kein App-Aufrufer mehr) entfernt; generische `makeProfileScopedValue<T>`-Factory für 7 von ~15 Getter/Setter-Paaren in `utils/storage.ts` (Language, Theme, ThemeName, TotalTasks, ChallengeHighScore, SoundsEnabled, SoundsVolume) eingeführt. `Operations`/`NumberRange` behalten bewusst ihre eigene Implementierung (Alt-Format-Migrationslogik). Punkte 1 (App.tsx God Component), 3 (useGameLogic.ts Rundenwechsel-Sequenz) und 4 (Design-Tokens) sowie der kritische CI-Gate-Befund (PR-Trigger nur für `main`, `|| true` maskiert TS-Fehler) aus dem Nachfass-Kommentar bleiben offen für Folge-PRs.
...
| #356 ✅      | fix: CI-Trigger auch für PRs gegen `testing` ausführen                                                                                                                                                   |
